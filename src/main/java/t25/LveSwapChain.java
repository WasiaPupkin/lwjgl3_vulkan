...
public class LveSwapChain {
    ...
    private final Vector<Long> swapChainFramebuffers = new Vector<>(); //LvePipline#createFramebuffers
    private long renderPass;
    private final Vector<Long> depthImageViews = new Vector<>(); //no?
    private LongBuffer pSwapchainImages;
    private final Vector<Long> swapChainImageViews = new Vector<>(); //imageViews
    private final LveDevice lveDevice;
    private final VkExtent2D windowExtent;
    private long swapChain;
    private LveSwapChain oldSwapChainObj;
    private final Vector<Long> imageAvailableSemaphores = new Vector<>(); //pImageAcquiredSemaphore
    private final Vector<Long> renderFinishedSemaphores = new Vector<>(); //pRenderCompleteSemaphore;
    private final Vector<Long> inFlightFences = new Vector<>();
    private final Vector<Long> imagesInFlight = new Vector<>();
    private AtomicInteger currentFrame = new AtomicInteger(0);

    public void onDestroy() {
        swapChainFramebuffers.forEach(buffer-> vkDestroyFramebuffer(lveDevice.getDevice(), buffer, null));
        swapChainImageViews.forEach(imageView-> vkDestroyImageView(lveDevice.getDevice(), imageView, null));
        depthImageViews.forEach(imageView-> vkDestroyImageView(lveDevice.getDevice(), imageView, null));
        destroySemaphores();
        inFlightFences.forEach(fence-> vkDestroyFence(lveDevice.getDevice(), fence, null));
        vkDestroyRenderPass(lveDevice.getDevice(), renderPass, null);
//        vkFreeMemory(lveDevice.getDevice(), swapChain, null);
    }
...
//    public void setRenderCommandBuffers(VkCommandBuffer[] renderCommandBuffers) {this.renderCommandBuffers = renderCommandBuffers;}
    ...
    public Map<String, Integer> acquireNextImage() {
        IntBuffer pCurrentImageIndex = memAllocInt(1);
        _CHECK_(vkWaitForFences(lveDevice.getDevice(), inFlightFences.get(currentFrame.get()), true, UINT64_MAX), "Cannot wait next image");

        int result = vkAcquireNextImageKHR(lveDevice.getDevice(), swapChain, UINT64_MAX, imageAvailableSemaphores.get(currentFrame.get()), VK_NULL_HANDLE, pCurrentImageIndex);
        int currentImageIndex = pCurrentImageIndex.get(0);
        memFree(pCurrentImageIndex);

        var ret = new HashMap<String, Integer>();
        ret.put("result", result);
        ret.put("currentImageIndex", currentImageIndex);

        return ret;
    }

    int submitCommandBuffers(PointerBuffer pCommandBuffers, int currentImageIndex) {
        if (imagesInFlight.get(currentImageIndex) != null) {
            vkWaitForFences(lveDevice.getDevice(), imagesInFlight.get(currentImageIndex), true, UINT64_MAX);
        }
        imagesInFlight.set(currentImageIndex, inFlightFences.get(currentFrame.get()));
...
//        pWaitDstStageMask.flip();
        ...
        pWaitSemaphore.put(0, imageAvailableSemaphores.get(currentFrame.get()));
//        pWaitSemaphore.flip();

        LongBuffer pSignalSemaphore = memAllocLong(1);
        pSignalSemaphore.put(0, renderFinishedSemaphores.get(currentFrame.get()));
//        pSignalSemaphore.flip();
        ...
        vkResetFences(lveDevice.getDevice(), inFlightFences.get(currentFrame.get()));

        _CHECK_(vkQueueSubmit(lveDevice.getGraphicsQueue(), submitInfo, inFlightFences.get(currentFrame.get())), "Can't submit graphics queue");

        LongBuffer pSwapchain = memAllocLong(1);
        pSwapchain.put(0, swapChain);
//        pSwapchain.flip();

        IntBuffer pImageIndex = memAllocInt(1).put(0, currentImageIndex);
        ...
        currentFrame.set(currentFrame.addAndGet(1) % MAX_FRAMES_IN_FLIGHT);

        memFree(pWaitSemaphore);
        memFree(pSignalSemaphore);
...
//        swapChainSupport.capabilities.free();
        ...
        if (oldSwapChainObj != null && oldSwapChainObj.swapChain != 0) {
            vkDestroySwapchainKHR(lveDevice.getDevice(), oldSwapChainObj.swapChain, null);
        }
...
//        System.out.println("SwapChain Present mode: " + swapchainPresentMode);
    ...
    private void createDepthResources() {
        ...
        for (int i = 0; i < imageCount(); i++) {
            ...
            long depthImage = lveDevice.createImageWithInfo(imageInfo, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
