...
public class LveGameObject {
    public static class PointLightComponent {
    ...
    public class TransformComponent {
        ...
        public Vector3f scale = new Vector3f(1.f, 1.f, 1.f);
...
//        }
        ...
        public Matrix3f normalMatrix() {
            ...
            float c1 = (float) Math.cos(rotation.y());
            float s1 = (float) Math.sin(rotation.y());
            Vector3f invScale = new Vector3f(1.0f).div(scale, new Vector3f());

            return new Matrix3f(
                    new Vector3f(scale.x() * (c1 * c3 + s1 * s2 * s3),
                            scale.x() * (c2 * s3),
                            scale.x() * (c1 * s2 * s3 - c3 * s1)),
                    new Vector3f(scale.y() * (c3 * s1 * s2 - c1 * s3),
                            scale.y() * (c2 * c3),
                            scale.y() * (c1 * c3 * s2 + s1 * s3)),
                    new Vector3f(scale.z() * (c2 * s1),
                            scale.z() * (-s2),
                            scale.z() * (c1 * c2))
            );
        }
    ...
    public static LveGameObject makePointLight(float intensity, float radius, Vector3f color) {
        float localIntensity = intensity != 0 ? intensity : 10.f;
        float localRadius = radius !=0 ? radius : 0.1f;
        Vector3f localColor = color !=null ? color : new Vector3f(1.f);

        LveGameObject gameObj = LveGameObject.createGameObject();
        gameObj.color = localColor;
        gameObj.transform.scale.x = localRadius;
        gameObj.pointLight = new PointLightComponent();
        gameObj.pointLight.lightIntensity = localIntensity;
        return gameObj;
    }
}
