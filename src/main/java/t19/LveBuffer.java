...
//        alignmentSize = getAlignment(instanceSize, minOffsetAlignmentLocal);
...
//        long offsetLocal = Math.max(offset, 0);+
     ...
     *
     */
    public void writeToIndex(long data, int index) {
    ...
    private static long getAlignment(long instanceSize, long minOffsetAlignment) {
        if (minOffsetAlignment > 0) {
            return (instanceSize + minOffsetAlignment - 1) & ~(minOffsetAlignment - 1);  //fixme whats that
        }
        return instanceSize;
    }
}
