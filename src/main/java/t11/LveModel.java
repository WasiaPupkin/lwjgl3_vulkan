...
//    }
    private float[][] toFloatArr(Vector<Vertex> vertices) {
        ...
        for (int i = 0; i < vertices.size(); i++) {
            mergedMatrices[i] = new float[]{vertices.get(i).position.x(), vertices.get(i).position.y()};
        }
    ...
    static class Vertex {
        ...
        public static VkVertexInputBindingDescription.Buffer getBindingDescriptions(){
            ...
            return VkVertexInputBindingDescription.calloc(1)
                    ...
                    .stride(sizeof(new Vector2f())) // offset for the next buffer element per vertex [(x,y),(r,g,b,a)] <-- that was 2+4
        ...
        public static VkVertexInputAttributeDescription.Buffer getAttributeDescriptions(){
            ...
            attributeDescriptions.get(0)
                    ...
                    .offset(offsetof(new Vector2f()));
            // Location 1 : Color
            attributeDescriptions.get(1)
                    .binding(0) // <- binding point used in the VkVertexInputBindingDescription
                    .location(1) // <- location in the shader's attribute layout (inside the shader source)
                    .format(VK_FORMAT_R32G32B32_SFLOAT)
                    .offset(offsetof(new Vector3f())); // color followed by verts. so to get it, we must offset verts in bytes
